...
static void _serialize_stream_event_context_default(
	struct tg_ctf_ctx *ctx,
	uint64_t sec_tid
)
{
	{
		...
		/* "tid" field */
		_ALIGN(ctx->at, 64);
		tg_ctf_bt_bitfield_write_le(&ctx->buf[_BITS_TO_BYTES(ctx->at)], uint8_t, 0, 64, sec_tid);
		ctx->at += 64;
	}
...
)
{
	...
	/* stream event context */
	_serialize_stream_event_context_default(ctx, sec_tid);
...
)
{
	...
	/* get event size */
	ev_size = _get_event_size_default_instr((void *) ctx, sec_tid, ep_pc);
	...
	/* serialize event */
	_serialize_event_default_instr((void *) ctx, sec_tid, ep_pc);
...
)
{
	...
	/* event payload */
	{
		...
		/* "op" field */
		/* field size: 8 (partial total so far: 72) */
...
)
{
	...
	/* get event size */
	ev_size = _get_event_size_default_mem((void *) ctx, sec_tid, ep_op, ep_addr, ep_size);
	...
	/* serialize event */
	_serialize_event_default_mem((void *) ctx, sec_tid, ep_op, ep_addr, ep_size);
